...
\SV
   ...
   // https://makerchip.com/sandbox/0wpfLh9vv/0lOhV9# Design implementation 
...
\TLV
   ...
   $imm[31:0]  = $is_i_instr ? { {21{$instr[31]}}, $instr[30:20] } :   // I-type
                 { {20 {$instr[3]}}, $instr[7], $instr[30:25], $instr[11:8], 1'b0} //TBD    // B-type
                ;   // Default (unused)
   ...
   $result[31:0] = $is_addi ? $src1_value + $imm :    // ADDI: src1 + imm
                   //TBD   // ADD: src1 + src2
                   $src1_value + $src2_value;   // Default (unused)
   ...
   // **Lab: Branch Target
   $br_target_pc[31:0] = $taken_branch ? $pc+$imm : $pc+1;
